<script setup>
import { deleteGoodAPI, getAllGoodAPI, getMyGoodAPI } from '@/apis/good'; // 获取我的商品列表 API
...
// 获取商品列表
const getAllGoodList = async () => {
    const res = await getAllGoodAPI(reqData.value);
    goodList.value = res.data.goods; // 获取商品列表
    total.value = res.data.total; // 获取商品总数
    for (let good of goodList.value) {
        const userRes = await getUserByIdAPI(good.seller)
        good.sellerSid = userRes.data.student_id
    }
};
                ...
                <div class="good-item" v-for="good in goodList" :key="good.id">
                    ...
                    <div class="body">
                        ...
                        <div class="column action">
                            <el-button @click="deleteGood(good.id)" type="primary"
                                v-if="good.is_effective">删除商品</el-button>
                            <p><a href="javascript:;" v-if="good.is_effective"
                                    @click="router.push(`/detail/${good.id}`)">查看详情</a>
                            </p>
                        </div>
                    </div>
                </div>
