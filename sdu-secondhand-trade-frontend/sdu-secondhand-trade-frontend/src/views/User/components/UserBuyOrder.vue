...
import { cancelOrderAPI, getBuyerOrderListAPI, sendAfterSaleAPI } from '@/apis/order';
...
const getBuyerOrderList = async () => {
  // 请求订单列表
  const res = await getBuyerOrderListAPI(reqData.value);
          ...
          <div class="order-item" v-for="order in orderList" :key="order.id">
            ...
            <div class="body" v-if="order.good">
              ...
              <div class="column action">
                ...
                <p v-if="[0, 1].includes(order.status)"><a href="javascript:;" @click="cancelOrder(order)">取消订单</a></p>
              </div>
            </div>
          </div>
          <!-- 分页 -->
